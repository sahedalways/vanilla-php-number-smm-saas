...
if ($order && in_array($order['status'], ['PENDING', 'RECEIVED'])) {
    // Call API to check SMS
    $api = new FiveSimApi();
    $apiResponse = $api->getSMS($orderIdStr);


    $orderStatus = $apiResponse['data']['status'] ?? $order['status'];
    $otpCode = $apiResponse['data']['sms'][0]['code'] ?? null;
    ...
    echo json_encode([
        'status' => 'error',
        'message' => 'Order not eligible for processing'
    ]);
}
